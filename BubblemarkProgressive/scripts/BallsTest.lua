...
function BallsTest:start() 
	...
	for i = 0,self._N-1,1 do
		self._balls[i] = Ball:init("ball"..i)
	end
...
function BallsTest:getFPS()
  ...
  return tostring(math.floor(fps))
...
function BallsTest:showFPS() 
	...
	local db_update = {}
	local msg = string.format("FPS [%d]: %d", self._N, math.floor(fps))
	db_update["fpsText"] = msg
