...
function Ball:init(controlName) 
	...
	local dk_data = {}
  dk_data["y"] = newBall._y
  dk_data["x"] = newBall._x
  dk_data["hidden"] = 0
  
  local data={}
  --cloning the control and setting the new name control to the show animation
  gre.clone_control("ball", controlName, "main_layer", dk_data)
  newBall._newName="main_layer."..controlName
	return newBall
end

function Ball:delete()
  gre.delete_control(self._newName)
end
